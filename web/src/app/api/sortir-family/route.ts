...
import { checkSortirFamilyRateLimit } from "@/utils/rateLimiter";
...
const SERVICE_TYPE = "sortir-family";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateCheck = checkSortirFamilyRateLimit(user.id, cleanIds.length);
    ...
    const costPerId = await getCostPerId(SERVICE_TYPE, 25);
    ...
    try {
      ...
      console.error("[Sortir Family DB Deduct Error]:", dbErr);
      return NextResponse.json(
    ...
    try {
      ...
      formData.append("action", "sortir_family");
      ...
      const categorized = categorizeSortirResult(botkitaData, "member");
      ...
      console.error("[BotKita Sortir Family Fetch Error]:", fetchErr?.message || "Connection failed");
    ...
    return safeErrorResponse(err, "Terjadi kesalahan server saat memproses sortir family.");
  }
}
